...
describe('hook.js logic', () => {
  ...
  describe('readStdin parsing', () => {
    ...
    it('returns empty object for invalid JSON', () => {
      let result = {};
      try { result = JSON.parse('not json {{{'); } catch { result = {}; }
      expect(result).toEqual({});
    });

    it('returns empty object for empty string', () => {
      let result = {};
      try { result = JSON.parse(''); } catch { result = {}; }
      expect(result).toEqual({});
    });
  });
  ...
  describe('checkConflict handler', () => {
    ...
    it('builds CONFLICT output and exits 1 when conflicts found', async () => {
      ...
      team.checkConflicts.mockResolvedValue({
        conflicts: [{
          owner_handle: 'alice', tool: 'cursor',
          files: ['api.js'], summary: 'Adding endpoints',
        }],
        locked: [],
      });
  ...
  describe('reportEdit handler', () => {
    it('calls reportFile and recordEdit in parallel', async () => {
      ...
      await Promise.all([
        team.reportFile('t_abc', filePath),
        team.recordEdit('t_abc', filePath),
      ]);
  ...
  describe('subcommand routing', () => {
    ...
    it('unknown subcommand exits 1', () => {
      expect(() => {
        ...
        if (!['check-conflict', 'report-edit', 'report-tool-use', 'session-start'].includes(subcommand)) {
