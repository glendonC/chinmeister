...
describe('context cache', () => {
  ...
  describe('refreshContext', () => {
    ...
    it('sets offline flag on API failure and logs warning', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('API unreachable'), expect.anything());
      consoleSpy.mockRestore();
    });
    ...
    it('does not log duplicate offline warnings', async () => {
      ...
      const offlineLogs = consoleSpy.mock.calls.filter(c =>
        typeof c[0] === 'string' && c[0].includes('API unreachable')
      );
