...
import { findTeamFile as findTeamFileShared } from '../../../shared/team-utils.js';
...
describe('teamHandlers', () => {
  ...
  describe('searchMemories', () => {
    ...
    it('handles multiple tags', async () => {
      ...
      expect(client.get).toHaveBeenCalledWith('/teams/t_abc/memory?q=test&tags=config%2Credis&limit=10');
    });
  ...
  describe('claimFiles', () => {
    it('calls POST /teams/:id/locks with files', async () => {
      ...
      expect(client.post).toHaveBeenCalledWith('/teams/t_abc/locks', { files: ['auth.js', 'db.js'] });
    });
  });
  ...
  describe('endSession', () => {
    it('calls POST /teams/:id/sessionend with session_id', async () => {
      ...
      expect(client.post).toHaveBeenCalledWith('/teams/t_abc/sessionend', { session_id: 'sess_123' });
    });
  });
  ...
  describe('reportModel', () => {
    it('calls PUT /teams/:id/sessionmodel with model', async () => {
      ...
      expect(client.put).toHaveBeenCalledWith('/teams/t_abc/sessionmodel', { model: 'claude-opus-4-6' });
    });
  });
});
