...
describe('hook.js entry point coverage', () => {
  ...
  it('exits 0 when configExists returns false (check-conflict)', async () => {
    ...
    const fakeStdin = new Readable({ read() { this.push(null); } });
    ...
    // Restore stdin
    Object.defineProperty(process, 'stdin', { value: originalStdin || process.stdin, writable: true, configurable: true });
  });
  ...
  it('runs check-conflict with no conflicts', async () => {
    ...
    fakeStdin.setEncoding = fakeStdin.setEncoding || function(enc) { this._readableState.encoding = enc; return this; };
    Object.defineProperty(process, 'stdin', { value: fakeStdin, writable: true, configurable: true });
  ...
  it('runs check-conflict with conflicts detected', async () => {
    ...
    fakeStdin.setEncoding = fakeStdin.setEncoding || function(enc) { return this; };
    Object.defineProperty(process, 'stdin', { value: fakeStdin, writable: true, configurable: true });
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('CONFLICT:'));
  ...
  it('runs session-start and displays context', async () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('chinwag team context'));
    expect(exitSpy).toHaveBeenCalledWith(0);
  });
