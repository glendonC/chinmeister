...
describe('openPath', () => {
  ...
  it('opens path on macOS', () => {
    ...
    expect(execFileSync).toHaveBeenCalledWith('open', ['/some/path'], expect.objectContaining({ stdio: 'ignore' }));
  ...
  it('opens path on Linux', () => {
    ...
    expect(result).toEqual({ ok: true });
    expect(execFileSync).toHaveBeenCalledWith('xdg-open', ['/some/path'], expect.objectContaining({ stdio: 'ignore' }));
  ...
  it('opens path on Windows', () => {
    ...
    expect(result).toEqual({ ok: true });
    expect(execFileSync).toHaveBeenCalledWith('cmd', ['/c', 'start', '', '/some/path'], expect.objectContaining({ stdio: 'ignore' }));
