...
describe('formatFiles edge cases', () => {
  ...
  it('handles mixed media and code with overflow', () => {
    expect(formatFiles(['a.js', 'b.ts', 'c.css', 'd.py', 'logo.png'])).toBe('a.js, b.ts + 2 more + 1 image');
  });
...
describe('smartSummary edge cases', () => {
  ...
  it('keeps meaningful summaries', () => {
    expect(smartSummary({ summary: 'Refactoring auth', files: ['src/auth.js'] })).toBe('Refactoring auth');
  });

  it('keeps summaries when multiple files exist', () => {
    expect(smartSummary({ summary: 'app.js changes', files: ['src/app.js', 'src/b.js'] })).toBe('app.js changes');
  });
});
...
describe('hasVisibleSessionActivity edge cases', () => {
  ...
  it('returns false for ended session with no activity', () => {
    expect(hasVisibleSessionActivity({ ended_at: '2026-01-01T00:00:00Z', edit_count: 0, files_touched: [] })).toBe(false);
  });

  it('returns true for ended session with edits', () => {
    expect(hasVisibleSessionActivity({ ended_at: '2026-01-01T00:00:00Z', edit_count: 1, files_touched: [] })).toBe(true);
  });

  it('returns true for ended session with files', () => {
    expect(hasVisibleSessionActivity({ ended_at: '2026-01-01T00:00:00Z', edit_count: 0, files_touched: ['x.js'] })).toBe(true);
  });
});
...
describe('buildCombinedAgentRows', () => {
  ...
  it('handles null inputs', () => {
    const rows = buildCombinedAgentRows({ managedAgents: null, connectedAgents: null, getToolName });
    expect(rows).toEqual([]);
  });
...
describe('buildDashboardView', () => {
  ...
  it('filters out dashboard and unknown tool agents', () => {
    const view = buildDashboardView({
      context: {
        members: [
          { agent_id: 'a', handle: 'u1', tool: 'claude-code', status: 'active', activity: {} },
          { agent_id: 'b', handle: 'u2', tool: 'dashboard', status: 'active', activity: {} },
          { agent_id: 'c', handle: 'u3', tool: 'unknown', status: 'active', activity: {} },
          { agent_id: 'd', handle: 'u4', tool: null, status: 'active', activity: {} },
        ],
      },
    });
  ...
  it('filters out non-active agents', () => {
    const view = buildDashboardView({
      context: {
        members: [
          { agent_id: 'a', handle: 'u1', tool: 'claude-code', status: 'active' },
          { agent_id: 'b', handle: 'u2', tool: 'cursor', status: 'idle' },
        ],
      },
    });
    expect(view.activeAgents).toHaveLength(1);
  ...
  it('detects file conflicts between different users', () => {
    const view = buildDashboardView({
      ...
      context: {
        members: [
          { agent_id: 'a', handle: 'alice', tool: 'claude-code', status: 'active', activity: { files: ['shared.js', 'unique-a.js'] } },
          { agent_id: 'b', handle: 'bob', tool: 'cursor', status: 'active', activity: { files: ['shared.js', 'unique-b.js'] } },
        ],
      },
    });
  ...
  it('does not create conflicts for files owned by a single agent', () => {
    const view = buildDashboardView({
      context: {
        members: [
          { agent_id: 'a', handle: 'alice', tool: 'claude-code', status: 'active', activity: { files: ['a.js'] } },
          { agent_id: 'b', handle: 'bob', tool: 'cursor', status: 'active', activity: { files: ['b.js'] } },
        ],
      },
    });
  ...
  it('shows recent sessions only when no active agents exist', () => {
    const view = buildDashboardView({
      context: {
        members: [],
        recentSessions: [
          { ended_at: null, edit_count: 3, files_touched: ['a.js'] },
        ],
      },
    });
  ...
  it('hides recent sessions when active agents exist', () => {
    const view = buildDashboardView({
      context: {
        members: [
          { agent_id: 'a', handle: 'u', tool: 'claude-code', status: 'active' },
        ],
        recentSessions: [
          { ended_at: null, edit_count: 3, files_touched: ['a.js'] },
        ],
  ...
  it('filters out invisible recent sessions', () => {
    const view = buildDashboardView({
      context: {
        members: [],
        recentSessions: [
          { ended_at: '2026-01-01', edit_count: 0, files_touched: [] },
        ],
      },
    });
    expect(view.showRecent).toBe(false);
  ...
  it('computes isTeam when multiple unique handles are present', () => {
    const view = buildDashboardView({
      context: {
        members: [
          { agent_id: 'a', handle: 'alice', tool: 'claude-code', status: 'active' },
          { agent_id: 'b', handle: 'alice', tool: 'cursor', status: 'active' },
        ],
      },
    });
    ...
    const teamView = buildDashboardView({
      context: {
        members: [
          { agent_id: 'a', handle: 'alice', tool: 'claude-code', status: 'active' },
          { agent_id: 'b', handle: 'bob', tool: 'cursor', status: 'active' },
        ],
      },
    });
  ...
  it('calculates tool counts', () => {
    const view = buildDashboardView({
      context: {
        members: [
          { agent_id: 'a1', handle: 'u', tool: 'claude-code', status: 'active' },
          { agent_id: 'a2', handle: 'u', tool: 'claude-code', status: 'active' },
          { agent_id: 'a3', handle: 'u', tool: 'cursor', status: 'active' },
        ],
      },
    });
