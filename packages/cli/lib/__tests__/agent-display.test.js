...
describe('getAgentTargetLabel', () => {
  ...
  it('returns handle and display combined', () => {
    expect(getAgentTargetLabel({ handle: 'alice', _display: 'Claude Code' })).toBe('alice (Claude Code)');
  });
...
describe('getRecentResultSummary', () => {
  ...
  it('returns outputPreview when available', () => {
    expect(getRecentResultSummary(
      { outputPreview: 'Done: 5 files changed' },
      null
    )).toBe('Done: 5 files changed');
  });
