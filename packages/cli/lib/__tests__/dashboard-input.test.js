...
describe('createInputHandler', () => {
  ...
  it('toggles memory view on "k" when memories exist', () => {
    ...
    expect(ctx.setView).toHaveBeenCalled();
  ...
  it('switches to sessions view on "s" when agents exist', () => {
    ...
    const handleInput = createInputHandler(ctx);
    handleInput('s', makeKey());
    expect(ctx.setView).toHaveBeenCalledWith('sessions');
  });
  ...
  it('does not switch to sessions view when no agents', () => {
    ...
    const handleInput = createInputHandler(ctx);
    handleInput('s', makeKey());
    expect(ctx.setView).not.toHaveBeenCalledWith('sessions');
  });
  ...
  it('returns to home on Escape in agent-focus view', () => {
    ...
    expect(ctx.setView).toHaveBeenCalledWith('home');
    expect(ctx.setFocusedAgent).toHaveBeenCalledWith(null);
  });
  ...
  it('toggles diagnostics on "l" in agent-focus view for managed agent', () => {
    ...
    expect(ctx.setShowDiagnostics).toHaveBeenCalled();
  });
  ...
  it('moves focus from input to agents on down arrow', () => {
    ...
    const handleInput = createInputHandler(ctx);
    handleInput('', makeKey({ downArrow: true }));
    expect(ctx.setMainFocus).toHaveBeenCalledWith('agents');
  });
  ...
  it('moves focus from agents to input on up arrow when at top', () => {
    ...
    const handleInput = createInputHandler(ctx);
    handleInput('', makeKey({ upArrow: true }));
    expect(ctx.setMainFocus).toHaveBeenCalledWith('input');
  });
  ...
  it('opens agent-focus on Enter when agent is selected', () => {
    ...
    const handleInput = createInputHandler(ctx);
    handleInput('', makeKey({ return: true }));
    expect(ctx.setFocusedAgent).toHaveBeenCalledWith(agent);
    expect(ctx.setView).toHaveBeenCalledWith('agent-focus');
  });
  ...
  it('navigates down in sessions view', () => {
    ...
    const handleInput = createInputHandler(ctx);
    handleInput('', makeKey({ downArrow: true }));
    expect(ctx.setSelectedIdx).toHaveBeenCalled();
  });
  ...
  it('opens agent-focus on Enter in sessions view', () => {
    ...
    const handleInput = createInputHandler(ctx);
    handleInput('', makeKey({ return: true }));
    expect(ctx.setFocusedAgent).toHaveBeenCalledWith(liveAgents[0]);
    expect(ctx.setView).toHaveBeenCalledWith('agent-focus');
  });
  ...
  it('navigates down in memory view', () => {
    const ctx = createMockContext({
      view: 'memory',
      visibleMemories: [{ id: '1', text: 'A' }, { id: '2', text: 'B' }],
    });
    const handleInput = createInputHandler(ctx);
    handleInput('', makeKey({ downArrow: true }));
...
describe('createCommandHandler', () => {
  ...
  it('flashes warning when no tools ready for /new', () => {
    ...
    const handler = createCommandHandler(mockCtx);
    handler('/new');
    expect(mockCtx.flash).toHaveBeenCalledWith(expect.stringContaining('No tools ready'), expect.any(Object));
  });
  ...
  it('handles /recheck command', () => {
    const handler = createCommandHandler(mockCtx);
    handler('/recheck');
    expect(mockCtx.agents.refreshManagedToolStates).toHaveBeenCalledWith({ clearRuntimeFailures: true });
    expect(mockCtx.integrations.refreshIntegrationStatuses).toHaveBeenCalledWith({ showFlash: true });
  });
  ...
  it('handles /knowledge command', () => {
    const handler = createCommandHandler(mockCtx);
    handler('/knowledge');
    expect(mockCtx.setView).toHaveBeenCalledWith('memory');
    expect(mockCtx.memory.setMemorySelectedIdx).toHaveBeenCalledWith(-1);
  });
  ...
  it('handles /sessions command', () => {
    const handler = createCommandHandler(mockCtx);
    handler('/sessions');
    expect(mockCtx.setView).toHaveBeenCalledWith('sessions');
  });
  ...
  it('handles /message command without addressable agent', () => {
    ...
    const handler = createCommandHandler(mockCtx);
    handler('/message');
    expect(mockCtx.flash).toHaveBeenCalledWith(expect.stringContaining('Select a live agent'), expect.any(Object));
  });
  ...
  it('falls through to hero input when no tool can launch', () => {
    ...
    expect(mockCtx.setHeroInput).toHaveBeenCalledWith('some arbitrary text');
    expect(mockCtx.setHeroInputActive).toHaveBeenCalledWith(true);
  });
