...
export function Dashboard({
  ...
  const commandEntries = useMemo(
    ...
    [
      agents.unavailableCliAgents,
      agents.managedToolStates,
  ...
  // ── Render ─────────────────────────────────────────
  // DashboardProvider gives nested components access to config and
  // navigate without threading them through every intermediate prop.
  const renderContent = () => {
    // Guards
    if (cols < MIN_WIDTH) {
      return (
  ...
  return (
    <DashboardProvider config={config} navigate={navigate}>
      {renderContent()}
    </DashboardProvider>
  );
}
