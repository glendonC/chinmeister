...
export function AgentFocusView({
  ...
  const freshAgent = focusedAgent._managed
    ...
    : combinedAgents.find((agent) => !agent._managed && agent.agent_id === focusedAgent.agent_id) ||
      focusedAgent;
  const isRunning = freshAgent._managed
  ...
  const outputLines =
    showDiagnostics && freshAgent._managed
    ...
    <Box flexDirection="column" paddingTop={1}>
      ...
      <Box>
        <Text color={statusColor} bold>
          {getAgentDisplayLabel(freshAgent, liveAgentNameCounts, combinedAgents)}
        </Text>
        ...
        <Box flexDirection="column" marginTop={1}>
          ...
          {outputLines.length > 0 ? (
            outputLines.map((line, idx) => (
              <Text key={`${freshAgent.id}-${idx}`} dimColor>
                {' '}
                {line}
              </Text>
            ))
